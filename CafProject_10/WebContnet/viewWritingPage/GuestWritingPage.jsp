...
<link href="<c:url value="../viewWritingPageCSS/WritingPageCss.css"/>" rel="stylesheet">
<link rel="stylesheet" type="text/CSS" href="/CafeProject_10/IncludeFileCSS/LoginOkHeader.css">
...
<%@ include file="/IncludeFile/LoginOkHeader.jsp" %>
 <main>
     	...
     	<c:if test="${empty wishCheck }">
	     	<div id="wishBox">
	     		<a href="iLikeIt.do?likeType=guest&likeId=${showDTO.id }" id="heart" style="color:gray;"><i class="fas fa-heart"></i></a>
	     		<h5 id="wishDesc">가고싶다</h5>
	     	</div>
     	</c:if>
     	
     	<c:if test="${not empty wishCheck }">
	     	<div id="wishBox">
	     		<a href="iLikeIt.do?likeType=guest&likeId=${showDTO.id }" id="heart" style="color: rgb(191, 162, 218);"><i class="fas fa-heart"></i></a>
	     		<h5 id="wishDesc">가고싶다</h5>
	     	</div>
     	</c:if>
	    ...
	    <section>
	        ...
	        <h4 class="menuInfo">영업시간</h4> <h4 class="cafeInfo" id="hours">${showDTO.cafeHours  }</h4><br/>
	        <h4 class="menuInfo">쉬는시간</h4> 
    ...
    <nav>
      ...
      <article>
        ...
        <c:if test="${showDTO.cafePicture1 != '/CafeProject_10/UploadFolder_Owner/null' }">
    	    <img src="${showDTO.cafePicture1 }" class="viewCafeImg" id="Img1">
    	</c:if>
        <c:if test="${not empty showDTO.cafePicture2}">
	        <img src="${showDTO.cafePicture2 }" class="viewCafeImg" id="Img2"> 
        </c:if>
        <c:if test="${not empty showDTO.cafePicture3}">
        	<img src="${showDTO.cafePicture3 }" class="viewCafeImg" id="Img3">
        </c:if>
