...
import com.ww.hj.command.search.searchCommand;
...
import com.www.hj.command.report.ReportCommand;
import com.www.hj.command.stamp.GuestStampCommand;
import com.www.hj.command.stamp.GuestStampUseCommand;
...
public class FrontController extends HttpServlet {
	...
	public void doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/////////
		//하트 (wish List) 담아주기
		else if(servletPath.equals("/LoginPage/iLikeIt.do")||servletPath.equals("/iLikeIt.do") )
		{
		...
		// 2. guest
		else if(servletPath.equals("/LoginPage/guestSaveStamp.do"))
		{
			String cafeNumber=request.getParameter("cafeNumber");
			String stampNum=request.getParameter("stampNum");
			request.setAttribute("stampNum", stampNum);
			request.setAttribute("cafeNumber", cafeNumber);
			cmd = new GuestStampUseCommand();
			cmd.excute(request, response);
			int stampFlag=(int)request.getAttribute("stampFlag");
			System.out.println("stampFlag3: "+stampFlag); //주석
			if(stampFlag==0)
			{
				viewPage="/AlertPage/GuestStampUsOk.jsp";
			}else {
				viewPage="/ErrorPage/GuestStampNumError.jsp";
			}
		}
		
		//검색
		else if(servletPath.equals("/LoginPage/search.do")||servletPath.equals("/search.do"))
		{
			...
			{
				viewPage="/AlertPage/searchAlert.jsp";
			}
			else 
			{
				...
				{
					viewPage="/SearchPage/searchOk.jsp";
				}
				else if(type.equals("guest"))
				{
					viewPage="/SearchPage/searchOkGuest.jsp";
				}
				else if(type.equals("owner"))
				{
					viewPage="/SearchPage/searchOkOwner.jsp";
				}
			}
		}
		
		//신고하기
		else if(servletPath.equals("/LoginPage/report.do")||servletPath.equals("/report.do"))
		{
			String reportId=request.getParameter("reportId");
			String reportType=request.getParameter("reportType");
			session.setAttribute("reportId", reportId);
			if(reportType.equals("guest"))
			{				
				viewPage="/ReportPage/GuestReport.jsp";
			}
			else if(reportType.equals("owner"))
			{
				viewPage="/ReportPage/OwnerReport.jsp";
			}
		}
		//신고 내용 입력
		//reportOk.do
		else if(servletPath.equals("/LoginPage/reportOk.do")||servletPath.equals("/reportOk.do"))
		{
			String reportType=request.getParameter("reportType");
			String reportContents=request.getParameter("reportContents"); //reportSelct
			String reportSelect=request.getParameter("reportSelct"); //reportSelct

			request.setAttribute("contents", reportContents);
			request.setAttribute("reportSelect", reportSelect);
			
			cmd = new ReportCommand();
			cmd.excute(request, response);
			if(reportType.equals("guest"))
			{				
				viewPage="/AlertPage/ReportOkGuest.jsp";
			}
			else if(reportType.equals("owner"))
			{
				viewPage="/AlertPage/ReportOkOwner.jsp";
			}
		}
