...
public class DAOStamp{
    ...
    public ArrayList<DTOWriteOwner> loadCafeInfo(String cafeNumberSet)
    {
    	...
    	try {
    		...
    		while(rs.next())
    		{
    			...
    			dto.setCafeNumber(rs.getString("no"));
    ...
    // 3. 스탬프 갯수 불러오기
    // select stamp,cafeNo from stamp where id="usb" and cafeno in(1,12);
    public  ArrayList<DTOStampGuest>  showStampNum(String userId)
    {
    	ArrayList<DTOStampGuest> stampNumList = new ArrayList<DTOStampGuest>();
    	DTOStampGuest stampNumDTO=null;
    	
    	final String CAFE_STAMP_SQL="select stamp,cafeNo from stamp where id=?";
    	Connection conn = getConnection();
    	PreparedStatement ps = null;
    	ResultSet rs = null;
    	try {
    		ps  = conn.prepareStatement(CAFE_STAMP_SQL);
    		ps.setString(1,userId);
    		rs=ps.executeQuery();
    		while(rs.next())
    		{
    			stampNumDTO=new DTOStampGuest();
    			stampNumDTO.setCafeNumber(rs.getString("cafeNo"));
    			stampNumDTO.setUserStamp(rs.getString("stamp"));
    			stampNumList.add(stampNumDTO);
    		}
    	}catch(SQLException e) {
    		e.printStackTrace();
    	} finally {
    ...
    // guest 스탬프 갯수 삭제
    // 1.스탬프 갯수 확인
    public int checkStampNum(String id, String cafeNo)
    {
    	int check=0;
    	final String CHECHK_STAMP_NUM_SQL="select stamp from stamp where id=? and cafeNo=?";
    	
    	Connection conn = getConnection();
    	PreparedStatement ps = null;
    	ResultSet rs = null;
    	try {
    		ps  = conn.prepareStatement(CHECHK_STAMP_NUM_SQL);
    		ps.setString(1, id);
    		ps.setString(2, cafeNo);
    		rs=ps.executeQuery();
    		while(rs.next())
    		{
    			check =rs.getInt("stamp");
    		}
    	}catch(SQLException e) {
    		e.printStackTrace();
    	} finally {
    ...
    // 보유한 스탬프 갯수가 크면 guest 스탬프 갯수 삭제
    public void decresstampNum(String stampNum,String CafeNo,String id)
    {
    	final String DECRES_STAMP_NUM_SQL="update stamp set stamp=stamp-? where cafeNo=? and id=?";
    	Connection conn = getConnection();
    	PreparedStatement ps = null;
    	int rs = 0;
    	try {
    		ps  = conn.prepareStatement(DECRES_STAMP_NUM_SQL);
    		ps.setString(1, stampNum);
    		ps.setString(2, CafeNo);
    		ps.setString(3, id);
    		rs=ps.executeUpdate();
    	}catch(SQLException e) {
