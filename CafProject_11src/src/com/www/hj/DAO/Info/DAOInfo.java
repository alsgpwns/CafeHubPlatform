...
public class DAOInfo {
    ...
    // login no저장하기
    public String ownerNumber(String userId)
    {
    	String no=null;
    	final String OWNER_NUMBER_SQL="select no from board where id=?";
    	Connection conn = getConnection();
    	PreparedStatement ps = null;
    	ResultSet rs = null;
    	
    	try {
    		ps = conn.prepareStatement(OWNER_NUMBER_SQL);
    		ps.setString(1, userId);
    		rs=ps.executeQuery();
    		if(rs.next())
    		{
    ...
    // login cafeName 저장하기
    public String ownerCafeName(String userId)
    {
    	String cafeName=null;
    	final String OWNER_CAFENAME_SQL="select cafeName from board where id=?";
    	Connection conn = getConnection();
    	PreparedStatement ps = null;
    	ResultSet rs = null;
    	
    	try {
    		ps = conn.prepareStatement(OWNER_CAFENAME_SQL);
    		ps.setString(1, userId);
  ...
  //게스트 id 중복 확인
    ...
    //hashTag 가져오기
//    public ArrayList<DTOWriteOwner> loadDTO(){
//    	ArrayList<DTOWriteOwner> tagList = new ArrayList<DTOWriteOwner>();
//    	Connection conn = getConnection();
//    	PreparedStatement ps =null;
//    	ResultSet rs = null;
//    	final String LIST_SQL="select id,cafeHashTag from board";
//    	DTOWriteOwner dto = null;
//    	try {
//    		ps=conn.prepareStatement(LIST_SQL);
//    		rs=ps.executeQuery();
...
//    		{
//    			dto = new DTOWriteOwner();
//    			dto.setCafeHashTag(rs.getString("cafeHashTag"));
//    			dto.setId(rs.getString("id"));
//    			tagList.add(dto);
//    		}
//    	}catch(SQLException e)
...
//    	}
//    	
//    	  int totalElements = tagList.size();// arrayList의 요소의 갯수를 구한다.
